				...
				if tv, ok := f.Pkg.TypesInfo().Types[expr]; ok {
					...
					if rangeOverMapName == nodeStringRangeOverMap(expr) && n.X.Pos() == expr.Pos() && mapString == "map" {
						w.onFailure(lint.Failure{
							Confidence: 1,
							Failure: fmt.Sprintf(
								"Non-deterministic iteration over map '%s' detected. "+
									"The map of type '%s' should not be iterated using 'for ... range', as it can lead to inconsistent behavior across peers or concurrent runs.",
								rangeOverMapName, tv.Type.String(),
							),
							Node:     n,
